...
def preprocess():
    with open('data/dating-full.csv', newline='') as csvfile:
        ...
        unq_count, lower_count, label_encodings, dicdata = preprocess_asgn4(fulldata)
        with open('data/dating.csv', 'w') as writefile:
            writer = csv.DictWriter(writefile, dicdata[0].keys())
            writer.writeheader()
            writer.writerows(dicdata[:6500])
    df = pd.read_csv('data/dating.csv')
    df = df.drop(unquote_col, axis=1)
    for c in continuos_valued:
        df[c] = pd.cut(df[c], 2, labels=[0,1])
    df.to_csv('data/dating.csv')
    test = df.sample(frac=0.2, replace=False, random_state=47)
